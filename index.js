...
📨 \`!dm @role <message>\` — DM a role`);
  ...
  if (content.startsWith('!rps ')) {
    ...
    const result =
      player===botPick
      ? 'Draw!'
      : (player==='rock'&&botPick==='scissors')||(player==='paper'&&botPick==='rock')||(player==='scissors'&&botPick==='paper')
      ? 'You win!' : 'I win!';
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'ticket_select') {
    ...
    const existing = interaction.guild.channels.cache.find(c => c.name.startsWith(`ticket-${user.username.toLowerCase()}`));
    if (existing) return interaction.reply({ content: `⚠️ You already have an open ticket: <#${existing.id}>`, ephemeral: true });
    ...
    await ch.send({ content: `🎫 <@${user.id}> created a ticket for **${opt.label}**. <@&${setup.viewerRoleId}>`, allowedMentions:{users:[user.id],roles:[setup.viewerRoleId]} });
    ...
    await ch.send({ content:'🗑️ Click to close & get transcript.', components:[deleteBtn] });
